...
setup-cpu: check-comfyui
	...
	@echo "Run 'make status' to verify installat.PHONY: help check-venv check-torch check-comfyui setup-cuda130 setup-cpu setup-xpu update clean run-cpu run-gpu status
...
run-gpu: check-venv check-torch
	...
	uv run external/ComfyUI/main.py --enable-manager --preview-method latent2rgbion"
