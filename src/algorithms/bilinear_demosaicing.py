    ...
    for i in range(1,height+1):
        for j in range(1,width+1):
            ...
            # edge (without corners)
                ...
                if ((i-1)%2 == 1-dy and (j-1)%2 == dx) or ((i-1)%2 == dy and (j-1)%2 == 1-dx): # green pixel
                    if ((i == 1 and dy == 0) # if the pixels of the first row are red and green
                        or (j == 1 and dx == 0) # if the pixels of the first column are red and green
                        or (i == height and ((dy == 0 and height%2 == 1) or (dy == 1 and height%2 == 0))) # if the pixels of the last row are red and green
                        or (j == width and ((dx == 0 and width%2 == 1) or (dx == 1 and width%2 == 0))) # if the pixels of the last column are red and green
                    ):
                        nbpix_g_r = 2
                        nbpix_g_b = 1
            ...
            # center of the image
                ...
                nbpix_r_g = 4
                nbpix_r_b = 4
                nbpix_g_r = 2 # if the pixel is green, it has 2 red pixels as direct neighbours
                nbpix_g_b = 2
                nbpix_b_r = 4
                nbpix_b_g = 4
            ...
            if (i-1)%2 == dy and (j-1)%2 == dx: # red pixel (we can't use "rgb_image[i, j, 0] != 0" because a pixel can have (R=0, G=0, B=0))
                # green
                demosaicing_image[i, j, 1] = (padded_rgb_image[i-1, j, 1] + padded_rgb_image[i+1, j, 1] + padded_rgb_image[i, j-1, 1] + padded_rgb_image[i, j+1, 1]) / nbpix_r_g
                # blue
                demosaicing_image[i, j, 2] = (padded_rgb_image[i-1, j-1, 2] + padded_rgb_image[i-1, j+1, 2] + padded_rgb_image[i+1, j-1, 2] + padded_rgb_image[i+1, j+1, 2]) / nbpix_r_b
            elif ((i-1)%2 == 1-dy and (j-1)%2 == dx) or ((i-1)%2 == dy and (j-1)%2 == 1-dx): # green pixel
                # red
                demosaicing_image[i, j, 0] = (padded_rgb_image[i-1, j, 0] + padded_rgb_image[i+1, j, 0] + padded_rgb_image[i, j-1, 0] + padded_rgb_image[i, j+1, 0]) / nbpix_g_r
                # blue
                demosaicing_image[i, j, 2] = (padded_rgb_image[i-1, j, 2] + padded_rgb_image[i+1, j, 2] + padded_rgb_image[i, j-1, 2] + padded_rgb_image[i, j+1, 2]) / nbpix_g_b
            else: # blue pixel : (i-1)%2 == 1-dy and (j-1)%2 == 1-dx
                # red
                demosaicing_image[i, j, 0] = (padded_rgb_image[i-1, j-1, 0] + padded_rgb_image[i-1, j+1, 0] + padded_rgb_image[i+1, j-1, 0] + padded_rgb_image[i+1, j+1, 0]) / nbpix_b_r
                # green
                demosaicing_image[i, j, 1] = (padded_rgb_image[i-1, j, 1] + padded_rgb_image[i+1, j, 1] + padded_rgb_image[i, j-1, 1] + padded_rgb_image[i, j+1, 1]) / nbpix_b_g
