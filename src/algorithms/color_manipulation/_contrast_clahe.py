...
def contrast_clahe_cv2(
    rgb_image: Tensor, clip_limit: float = 2.0, grid_size: tuple[int, int] = (8, 8)
) -> Tensor:
    """
    ...
    out = (torch.from_numpy(rgb_eq).to(device=device, dtype=dtype) / 255.0).clamp(
        0.0, 1.0
    )
...
def contrast_clahe_kornia(
    rgb_image: Tensor, clip_limit: float = 4.0, grid_size: tuple[int, int] = (8, 8)
) -> Tensor:
    """
    ...
    # CLAHE
    L_eq = K.enhance.equalize_clahe(
        L_norm, clip_limit, grid_size
    )  # Ici, equalize_clahe ne voit qu’un seul canal, donc elle ne modifie que la luminance.

    # Reconstruction Lab
    L_eq = (L_eq * 100.0).clamp(0.0, 100.0)  # in [0,100]
