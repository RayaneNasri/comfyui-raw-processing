...
def selectReference(burstPath, imageList, options):
    ...
    if options["mode"] == "full" or options["mode"] == "align":
        if options["referenceIndex"] == -1:
            ...
            type(options["referenceIndex"]) == int
            and options["referenceIndex"] >= 0
            and options["referenceIndex"] < len(imageList)
        ):
...
def alignBurst(burstPath, rawPathList, images, refIdx, params, options):
    ...
    imagesPadded = [
        np.pad(
            im, ((paddingTop, paddingBottom), (paddingLeft, paddingRight)), "symmetric"
        )
        for im in images
    ]
    ...
    if params["writeMotionFields"]:
        ...
        outputName = os.path.join(
            alignedFolder,
            os.path.splitext(os.path.basename(rawPathList[refIdx]))[0] + ".jpg",
        )
        cv2.imwrite(
            outputName,
            cv2.cvtColor(referenceImage, cv2.COLOR_RGB2BGR),
        ...
        unpaddedMotionVectors = motionVectors[
            :, unpadVectorsTop:-unpadVectorsBottom, unpadVectorsLeft:-unpadVectorsRight
        ]
        ...
        for i, raw in enumerate(alternateRaws):
            ...
            outputName = os.path.join(
                alignedFolder,
                os.path.splitext(os.path.basename(raw))[0] + "_motion.jpg",
            )
            cv2.imwrite(
                outputName,
                cv2.cvtColor(alternateImage, cv2.COLOR_RGB2BGR),
                [cv2.IMWRITE_JPEG_QUALITY, 100],
            )
        if verbose:
            currentTime = getTime(
    ...
    if params["writeAlignedTiles"]:
        ...
        shutil.copy2(
            rawPathList[refIdx],
            os.path.join(alignedFolder, os.path.basename(rawPathList[refIdx])),
        )
...
def alignHdrplus(referenceImage, alternateImages, params, options):
    ...
    refTiles = getTiles(
        referenceImage, tileSize, tileSize // 2
    ...
    for i, alternateImage in enumerate(alternateImages):
        ...
        if verbose:
            currentTime = getTime(
                currentTime,
                " -- Aligned frame {}/{} to the reference image".format(
                    i + 1, len(alternateImages)
                ),
            )
...
def upsampleAlignments(
    ...
    upsampledAlignments = previousAlignments.repeat(repeatFactor, 0).repeat(
        repeatFactor, 1
    )
    ...
    neighborsMask = np.tile(
        tile,
        (
            upsampledAlignments.shape[0] // repeatFactor,
            upsampledAlignments.shape[1] // repeatFactor,
            1,
            1,
        ),
    )
    ...
    ti1 = np.repeat(
        np.clip(
            2 + np.arange(h) // repeatFactor + neighborsMask[:, 0, 0, 0],
            0,
            paddedPreviousAlignments.shape[0] - 1,
        ).reshape(h, 1),
        w,
        axis=1,
    ).reshape(h * w)
    ti2 = np.repeat(
        np.clip(
            2 + np.arange(h) // repeatFactor + neighborsMask[:, 0, 1, 0],
            0,
            paddedPreviousAlignments.shape[0] - 1,
        ).reshape(h, 1),
        w,
    ...
    tj1 = np.repeat(
        np.clip(
            2 + np.arange(w) // repeatFactor + neighborsMask[0, :, 0, 1],
            0,
            paddedPreviousAlignments.shape[1] - 1,
        ).reshape(1, w),
        h,
        axis=0,
    ).reshape(h * w)
    tj2 = np.repeat(
        np.clip(
            2 + np.arange(w) // repeatFactor + neighborsMask[0, :, 1, 1],
            0,
            paddedPreviousAlignments.shape[1] - 1,
        ).reshape(1, w),
        h,
    ...
    selectedAlignments = candidateAlignments[
        np.arange(h * w), np.argmin([d0, d1, d2], axis=0)
    ].reshape((h, w, 2))
    if (
        h < referencePyramidLevel.shape[0] // (tileSize // 2) - 1
        or w < referencePyramidLevel.shape[1] // (tileSize // 2) - 1
    ):
        # tiles were no alignment was computed will have an estimated motion of 0 pixels
        newAlignments = np.zeros(
            (
                referencePyramidLevel.shape[0] // (tileSize // 2) - 1,
                referencePyramidLevel.shape[1] // (tileSize // 2) - 1,
                2,
            ),
            dtype=selectedAlignments.dtype,
        )
...
def alignOnALevel(
    ...
    upsamplingFactor=1,
    tileSize=16,
    previousTileSize=None,
    searchRadius=4,
    distance="L2",
    subpixel=True,
    previousAlignments=None,
):
    ...
    # For convenience
    h, w, sR = (
        upsampledAlignments.shape[0],
        upsampledAlignments.shape[1],
        2 * searchRadius + 1,
    )
    ...
    searchAreas = getTiles(
        np.pad(
            alternatePyramidLevel,
            searchRadius,
            mode="constant",
            constant_values=2**16 - 1,
        ),
        window=tileSize + 2 * searchRadius,
    )
    # Only keep those corresponding to the area around the reference tile location + [u0, v0]
    indI = np.clip(
        (
            (
                np.repeat((np.arange(h) * tileSize // 2).reshape(h, 1), w, axis=1)
            ).reshape(h, w)
            + u0
        ).reshape(h * w),
        0,
        searchAreas.shape[0] - 1,
    )
    indJ = np.clip(
        (
            (
                np.repeat((np.arange(w) * tileSize // 2).reshape(1, w), h, axis=0)
            ).reshape(h, w)
            + v0
        ).reshape(h * w),
        0,
        searchAreas.shape[1] - 1,
    )
    ...
    minIdx = np.argmin(distances, axis=1).astype(
        np.uint16
    )  # distance minimum indices are flattened

    levelOffsets = np.zeros(
        (upsampledAlignments.shape[0] * upsampledAlignments.shape[1], 2),
        dtype=np.float32,
    )
