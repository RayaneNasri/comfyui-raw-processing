...
from numba import guvectorize, vectorize, float32, float64, uint16
...
def fGammaCompress_(x, threshold, gainMin, gainMax, exponent):
    ...
    if x <= threshold:
        ...
        x = gainMax * (x**exponent) - gainMax + 1.0
    # Clip
...
def fGammaDecompress_(x, threshold, gainMin, gainMax, exponent):
    ...
    if x <= threshold:
        ...
        x = ((x + gainMax - 1.0) / gainMax) ** exponent
    # Clip
    return 0 if x < 0 else (1 if x > 1 else x)
...
def gammasRGB(image, mode="compress"):
    ...
    if mode == "compress":
        if np.issubdtype(image.dtype, np.unsignedinteger):
            return uGammaCompress_(image, 0.0031308, 12.92, 1.055, 1.0 / 2.4)
        else:
            return fGammaCompress_(image, 0.0031308, 12.92, 1.055, 1.0 / 2.4)
    else:
        if np.issubdtype(image.dtype, np.unsignedinteger):
...
def gammarec709(image, mode="compress"):
    ...
    if mode == "compress":
        if np.issubdtype(image.dtype, np.unsignedinteger):
            return uGammaCompress_(image, 0.018, 4.5, 1.099, 1.0 / 2.2)
        else:
            return fGammaCompress_(image, 0.018, 4.5, 1.099, 1.0 / 2.2)
    else:
        if np.issubdtype(image.dtype, np.unsignedinteger):
...
def rgb2YUV(rgbImage):
    ...
    assert (
        np.issubdtype(rgbImage.dtype, np.floating)
        and np.min(rgbImage) >= 0.0
        and np.max(rgbImage) <= 1.0
    ), "expected a float image between 0. and 1."
    cvMat = np.array(
        [
            [0.299, 0.587, 0.114],
    ...
    return np.dot(rgbImage, cvMat.T).clip(0.0, 1.0)


def yuv2RGB(yuvImage):
    assert len(yuvImage.shape) == 3, "not 3 channel image"
    assert (
        np.issubdtype(yuvImage.dtype, np.floating)
        and np.min(yuvImage) >= 0.0
        and np.max(yuvImage) <= 1.0
    ), "expected a float image between 0. and 1."
    cvMat = np.array([[1, 0, 1.13983], [1, -0.39465, -0.58060], [1, 2.03211, 0]])
    return np.dot(yuvImage, cvMat.T).clip(0.0, 1.0)
...
def localToneMap(burstPath, mergedImage, options):
    ...
    if options["ltmGain"] == -1:
        dsFactor = 25
        shortS = cv2.resize(
            shortGray, (0, 0), fx=1 / dsFactor, fy=1 / dsFactor
        ).flatten()
        bestGain = False
        gain, compression, saturated = 0, 1.0, 0.0
        ...
        while (compression < 1.9 and saturated < 0.95) or (
            ...
            gain += 2
            longSg = gammasRGB(gain * shortS, "compress").clip(0.0, 1.0)
    ...
    longGray = meanGain_(
        mergedImage[:, :, 0], mergedImage[:, :, 1], mergedImage[:, :, 2], gain
    )
    if verbose:
    ...
    fusedg = mergeMertens.process(
        [
            (255.0 * shortg).astype(np.float32),
            (255.0 * longg).astype(np.float32),
        ]
    )  # .clip(0., 1.)
    if verbose:
    ...
    ltmImage = np.empty_like(mergedImage)
    applyScaling_(mergedImage, shortGray, fusedGray, ltmImage)
    if verbose:
...
def enhanceContrast(image, options):
    ...
    # apply an S-shaped contrast enhancement curve
    assert options["gtmContrast"] >= 0 and options["gtmContrast"] <= 1, (
        "expected a contrast enhancement ratio between 0 and 1"
    )
    assert (
...
def finish(burstPath, rawPathList, refIdx, mergedBayer, params, options):
    ...
    burstName = os.path.basename(burstPath)
    processedRef = (
        None  # initialized here; assigned below if any reference output is requested
    )
    ...
    if (
        params["writeReferenceImage"]
        or params["writeGammaReference"]
        or params["writeReferenceFinal"]
    ):
        with rawpy.imread(rawPathList[refIdx]) as rawReference:
    ...
    gain = None  # assigned by localToneMap if ltmGain is set
    if options["ltmGain"]:
        # hdr tone mapping / local tone mapping
        processedImage, gain, shortExposure, longExposure, fusedExposure = localToneMap(
            burstPath, processedImage, options
        )
        if verbose:
        ...
        if params["writeLongExposure"]:
            ...
            outputName = os.path.join(
                options["outputFolder"], burstName + "_longGain{}.jpg".format(gain)
            )
            cv2.imwrite(
                outputName,
                cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGR),
        ...
        if params["writeFusedExposure"]:
            ...
            outputName = os.path.join(
                options["outputFolder"], burstName + "_fusedGain{}.jpg".format(gain)
            )
            cv2.imwrite(
                outputName,
                cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGR),
        ...
        if params["writeLTMImage"]:
            outputImage = convert8bit_(processedImage)
            outputName = os.path.join(
                options["outputFolder"], burstName + "_ltmGain{}.jpg".format(gain)
            )
            cv2.imwrite(
                outputName,
                cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGR),
        ...
        if params["writeLTMGamma"]:
            ...
            outputImage = convert8bit_(outputImage)
            outputName = os.path.join(
                options["outputFolder"], burstName + "_ltmGain{}_gamma.jpg".format(gain)
            )
            cv2.imwrite(
                outputName,
                cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGR),
    ...
    if params["writeGTMImage"]:
        ...
        outputName = os.path.join(
            ...
            burstName
            + "_{}gamma.jpg".format(
                ("contrast{}_".format(params["gtmContrast"]))
                if params["gtmContrast"]
                else ""
            ),
        )
        cv2.imwrite(
    ...
    if params["writeReferenceFinal"]:
        ...
        # save image
        outputName = os.path.join(
            options["outputFolder"], burstName + "_reference_final.jpg"
        )
        cv2.imwrite(
            outputName,
            cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGR),
