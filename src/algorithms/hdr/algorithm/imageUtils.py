...
from numba import uint8, uint16, float32, float64
...
def downsample(image, kernel="gaussian", factor=2):
    ...
    if kernel is None:
        ...
        filteredImage = gaussian_filter(
            image, sigma=factor * 0.5, order=0, output=None, mode="reflect"
        )
    elif kernel == "bayer":
    ...
    if isTypeInt(image):
        return np.rint(
            filteredImage[: h2 * factor : factor, : w2 * factor : factor]
        ).astype(image.dtype)
    else:
...
def getAlignedTiles(image, tileSize, motionVectors):
    ...
    indIm = (
        np.round(
            (
                np.repeat((np.arange(hm) * tileSize // 2).reshape(hm, 1), wm, axis=1)
                + motionVectors[:, :, 0]
            )
        )
        .astype(int)
        .clip(0, image.shape[0] - tileSize)
    )
    indJm = (
        np.round(
            (
                np.repeat((np.arange(wm) * tileSize // 2).reshape(1, wm), hm, axis=0)
                + motionVectors[:, :, 1]
            )
        )
        .astype(int)
        .clip(0, image.shape[1] - tileSize)
    )
    indI = np.repeat((np.arange(h) * tileSize // 2).reshape(h, 1), w, axis=1)
    indJ = np.repeat((np.arange(w) * tileSize // 2).reshape(1, w), h, axis=0)
    indI[: indIm.shape[0], : indIm.shape[1]] = (
        indIm  # tiles that have no motion vectors attached will remain identical
    )
    indJ[: indJm.shape[0], : indJm.shape[1]] = indJm

    # Only keep the tiles we're interested in (step = tileSize // 2)
    alignedTiles = imageTiles[indI.reshape(h * w), indJ.reshape(h * w)].reshape(
        (h, w, tileSize, tileSize)
    )
...
def computeTilesDistanceL1_(refTiles, altTiles, offsets, res):
    ...
    h, w, _ = offsets.shape
    # res is already automatically allocated by Numba based on the signature
...
@guvectorize(
    ["void(float32[:, :], uint16[:], float32[:], float32[:])"],
    "(m, n), (m) -> (m), (m)",
)
def subPixelMinimum_(arrDst, arrIdx, resI, resJ):
    ...
    fA11 = [0.250, -0.50, 0.250, 0.50, -1.0, 0.50, 0.250, -0.50, 0.250]
    fA22 = [0.250, 0.50, 0.250, -0.50, -1.0, -0.50, 0.250, 0.50, 0.250]
    fA12 = [0.250, 0.00, -0.250, 0.00, 0.0, 0.00, -0.250, 0.00, 0.250]
    fb1 = [-0.125, 0.00, 0.125, -0.25, 0.0, 0.25, -0.125, 0.00, 0.125]
    fb2 = [-0.125, -0.25, -0.125, 0.00, 0.0, 0.00, 0.125, 0.25, 0.125]
...
def getTiles(a, window, steps=None, axis=None):
    ...
    if axis is not None:
        ...
        assert np.all(np.isin(axs, np.arange(ashp.size))), "Axes out of range"
    else:
