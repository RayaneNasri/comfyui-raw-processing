...
def mergeBurst(
    ...
    if params["writeMergedBayer"]:
        ...
        shutil.copy2(
            rawPathList[referenceIndex],
            os.path.join(mergedFolder, os.path.basename(rawPathList[referenceIndex])),
        )
    ...
    if params["writeReferenceImage"]:
        ...
        cv2.imwrite(
            outputName,
            cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGR),
    ...
    if params["writeGammaReference"]:
        ...
        outputName = os.path.join(
            options["outputFolder"], burstName + "_reference_gamma.jpg"
        )
        cv2.imwrite(
            outputName,
            cv2.cvtColor(outputImage, cv2.COLOR_RGB2BGR),
    ...
    if params["writeGammaMerged"]:
        ...
        if verbose:
            currentTime = getTime(
                currentTime, " -- Post-processed merged image w. gamma"
            )
        # save image
        outputName = os.path.join(
            options["outputFolder"], burstName + "_merged_gamma.jpg"
        )
...
def centeredCosineWindow(x, windowSize=16):
    ...
    y = 1 / 2 - 1 / 2 * np.cos(2 * np.pi * (x + 1 / 2.0) / windowSize)
    return y
...
def cosineWindow2Dpatches(patches):
    ...
    lineWeights = (
        centeredCosineWindow(np.arange(windowSize), windowSize)
        .reshape(-1, 1)
        .repeat(windowSize, 1)
    )
...
def depatchifyOverlap(patches):
    ...
    dp = patchSize // 2
    assert patchSize == patches.shape[-2] and patchSize % 2 == 0, (
    ...
    reconstructedImage = np.zeros(
        ((patches.shape[0] + 1) * dp, (patches.shape[1] + 1) * dp), dtype=imSet00.dtype
    )
    ...
    reconstructedImage[dp : imSet11.shape[0] + dp, dp : imSet11.shape[1] + dp] += (
        imSet11
    )
...
def patchesRMS(patches):
    ...
    # flatten each patch
    patches = np.reshape(
        patches, tuple(patches.shape[:-2] + (patches.shape[-2] * patches.shape[-1],))
    )
...
@vectorize(
    [
        ...
        complex64(complex64, complex64, float32),
    ],
    target="parallel",
)
...
def mergeWienerDFTPatches(
    referenceImagePatchesFFT, alternateImagePatchesFFT, noiseVariance, temporalFactor=8
):
    ...
    mergedImagePatchesFFT = mergeWienerDFTPatches_(
        referenceImagePatchesFFT, alternateImagePatchesFFT, noiseScaling * noiseVariance
    )

    return mergedImagePatchesFFT


def temporalDenoisePairPatches(
    referenceImagePatches,
    alternateImagePatches,
    noiseVariance,
    temporalFactor=8,
    method="DFTWiener",
):
    ...
    if method == "keepAlternate":
        ...
        mergedImagePatches = np.multiply(
            0.5, referenceImagePatches + alternateImagePatches
        )
    elif method == "DFTWiener":
        altPatchesFFT = torch.fft.fftn(
            torch.from_numpy(alternateImagePatches.astype(np.float32)), dim=(2, 3)
        ).numpy()
        mergedImagePatches = mergeWienerDFTPatches(
            referenceImagePatches, altPatchesFFT, noiseVariance, temporalFactor
        )
    else:
...
@vectorize(
    [complex128(complex128, float64), complex64(complex64, float32)], target="parallel"
)
...
def spatialDenoisePatches(patchesFFT, noiseVariance, spatialFactor):
    ...
    rowDistances = (
        (np.arange(patchSize) - patchSize / 2).reshape(-1, 1).repeat(patchSize, 1)
    )
    ...
    return spatialDenoisePatches_(
        patchesFFT, distPatchShift * noiseScaling * noiseVariance
    )
...
def getNoiseParams(tags, blackLevel, whiteLevel, params, options):
    ...
    if type(params["noiseCurve"]) is tuple:
        ...
        if params["noiseCurve"] == "exifNoiseProfile":
            ...
            if "Image Tag 0xC761" in tags:
                ...
                if len(noiseProfile) == 2:
                    ...
                    assert noiseProfile[0] == noiseProfile[2] == noiseProfile[4], (
                        "NoiseProfile tag is different for each channel"
                    )
                    assert noiseProfile[1] == noiseProfile[3] == noiseProfile[5], (
                        "NoiseProfile tag is different for each channel"
                    )
                    lambdaSn = noiseProfile[0]
                    lambdaRn = noiseProfile[1]
                ...
                if options["verbose"] > 1:
                    print(
                        "NoiseProfile tag not found. Computing lambdaS and lambdaR from ISO({}) and baseline values".format(
                            ISO
                        )
                    )
                baselineLambdaS, baselineLambdaR = (
                    3.24 * 10 ** (-4),
                    4.3 * 10 ** (-6),
            ...
            # Removed unused baselineNoise assignment
            ISO = 100
            if "Image ISOSpeedRatings" in tags:
                ISO = tags["Image ISOSpeedRatings"].values[0]
            elif "EXIF ISOSpeedRatings" in tags:
            ...
            if options["verbose"] > 1:
                print(
                    "Computing lambdaS and lambdaR from ISO({}) and baseline values".format(
                        ISO
                    )
                )
            baselineLambdaS, baselineLambdaR = 3.24 * 10 ** (-4), 4.3 * 10 ** (-6)
            lambdaSn = ISO / 100 * baselineLambdaS
            lambdaRn = (ISO / 100) ** 2 * baselineLambdaR
        else:
            raise ValueError(f"Unsupported noiseCurve type: {params['noiseCurve']}")
        ...
        lambdaS, lambdaR = (
            lambdaSn * (whiteLevel - bL),
            lambdaRn * (whiteLevel - bL) ** 2,
        )
...
def mergeChannelHdrplus(
    referenceChannel, alignedChannelTiles, lambdaS, lambdaR, params, options
):
    ...
    if params["method"] == "DFTWiener":
        ...
        noiseVariance = (
            (np.multiply(lambdaS, signalLevel[..., np.newaxis, np.newaxis]) + lambdaR)
            .repeat(params["patchSize"], axis=-2)
            .repeat(params["patchSize"], axis=-1)
        )
        if verbose:
            currentTime = getTime(currentTime, " ---- Get noise variance")
        # compute 2D DFT of reference image tiles
        referenceChannelTiles = torch.fft.fftn(
            torch.from_numpy(referenceChannelTiles.astype(np.float32)), dim=(2, 3)
        ).numpy()
        if verbose:
    ...
    if options["temporalFactor"] == 0:
        ...
        for i in range(1, len(alignedChannelTiles)):
            mergedPairsTilesSum += temporalDenoisePairPatches(
                referenceChannelTiles,
                alignedChannelTiles[i],
                noiseVariance,
                options["temporalFactor"],
                params["method"],
            )
        if verbose:
    ...
    if params["method"] == "DFTWiener":
        if options["spatialFactor"] > 0:
            if noiseVariance is None:
                raise ValueError("noiseVariance was not initialized.")
            ...
            mergedChannelTiles = spatialDenoisePatches(
                mergedChannelTiles, noiseVariance, options["spatialFactor"]
            )
            if verbose:
                currentTime = getTime(currentTime, " ---- Spatial denoise")

        mergedChannelTiles = torch.fft.ifftn(
            torch.from_numpy(mergedChannelTiles), dim=(2, 3)
        ).real.numpy()
        if verbose:
...
def mergeHdrplus(
    referenceImage, alignedTiles, padding, tags, blackLevel, whiteLevel, params, options
):
    ...
    h, w = (
        (alignedTiles.shape[1] + 1) * tileSize // 2,
        (alignedTiles.shape[2] + 1) * tileSize // 2,
    )
    ...
    for c, (di, dj) in enumerate(zip([0, 1, 0, 1], [0, 0, 1, 1])):
        ...
        mergedImage[di::2, dj::2] = mergeChannelHdrplus(
            referenceImage[di::2, dj::2],
            alignedTiles[..., di::2, dj::2],
            lambdaS,
            lambdaR,
            params,
            options,
    ...
    return mergedImage[
        padding[0] : h - padding[1], padding[2] : w - padding[3]
    ]  # padding discarded here
