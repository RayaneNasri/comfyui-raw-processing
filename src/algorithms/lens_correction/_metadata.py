...
def try_read_vignette_gain_map(
    ...
    Looks for a GainMap opcode (ID 9) in the DNG tags.
    The raw map is bilinearly upsampled to (target_h, target_w, 3).

    Returns None silently if the path is empty, the file has no GainMap, or
    any parsing error occurs, the caller then falls back to manual parameters.

    Args:
        image_path: Path to the RAW/DNG file.  Empty string or None → None.
        target_h:   Target height to upsample the gain map to.
        target_w:   Target width to upsample the gain map to.

    Returns:
        Tensor (H, W, 3) of multiplicative gain values, or None.
    """
    if not image_path:
        return None
...
def _assemble_gain_map(
    ...
    """Convert raw gain map array(s) to a (target_h, target_w, 3) RGB tensor."""
    try:
