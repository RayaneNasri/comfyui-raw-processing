...
class BilinearDemosaicNode:
    ...
    def INPUT_TYPES(s):
        return {
            "required": {
                "bayer_img": ("IMAGE",),    # Shape: [1, H, W, 1] (from read_raw)
                "cfa_pattern": ("MASK",),   # Shape: [1, H, W]    (from read_raw)
            }
        }
    ...
    def execute(self, bayer_img, cfa_pattern):
        ...
        # 2. CALCULUS: Find dx and dy inside the node
        ...
        # 3. PRE-PROCESS: Convert 1-channel Bayer to 3-channel sparse RGB
        # Your function expects a (H, W, 3) tensor where only the sampled 
        ...
        sparse_rgb[1-detected_dy::2, 1-detected_dx::2, 2] = bayer_2d[1-detected_dy::2, 1-detected_dx::2] # Blue
        ...
        # 4. EXECUTE: Call your original function
        result = bilinear_demosaicing(sparse_rgb, dx=detected_dx, dy=detected_dy)

        # 5. POST-PROCESS: Return to ComfyUI format [1, H, W, 3]
        return (result.unsqueeze(0),)

NODE_CLASS_MAPPINGS = {
    "BilinearDemosaicNode": BilinearDemosaicNode
}

NODE_DISPLAY_NAME_MAPPINGS = {
    "BilinearDemosaicNode": "Bilinear Demosaicing (Manual Logic)"
}