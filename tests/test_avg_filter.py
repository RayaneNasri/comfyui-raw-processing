...
@pytest.mark.parametrize(
    "dtype", [torch.float16, torch.float64, torch.int32, torch.int64, torch.bool]
)
def test_avg_filter_unusual_dtypes(dtype):
    ...
    try:
        ...
        assert result.dtype == torch.float32, (
            "Data type was unexpectedly altered in a destructive way"
        )
...
def test_avg_filter_extreme_kernel_size():
    ...
    assert result.shape == img.shape, (
        "Shape should remain constant even with massive kernels"
    )
