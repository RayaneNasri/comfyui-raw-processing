...
from src.algorithms.denoising import bilateral_filter 
...
@pytest.mark.parametrize("invalid_sigma", [
    -1.0,         # Negative value
...
@pytest.mark.parametrize("invalid_sigma_type", [
    "75.0",       # String
    [75.0],       # List
    None          # NoneType
])
...
@pytest.mark.parametrize("anomalous_sigma", [
    float('inf'), # Infinity
...
def test_bilateral_filter_invalid_border_type(border_type):
    ...
    with pytest.raises((ValueError, TypeError, KeyError)):
        bilateral_filter(img, d=5, sigmaColor=75.0, sigmaSpace=75.0, strBorderType=border_type)
...
def test_bilateral_filter_gpu_tensor():
    ...
    try:
        ...
        pytest.fail("Function crashed on GPU tensor. The .cpu() logic failed or was bypassed.")
