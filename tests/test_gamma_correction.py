...
def test_gamma_correction_known_values():
    ...
    assert torch.allclose(out, expected, atol=1e-5), (
        "The (1/gamma) power calculation is incorrect."
    )
...
def test_gamma_correction_zero_gamma_raises_error():
    ...
    with pytest.raises((ZeroDivisionError, RuntimeError, ValueError)):  # type: ignore
...
def test_gamma_correction_rejects_wrong_type():
    ...
    with pytest.raises((TypeError, AttributeError)):  # type: ignore
        gamma_correction(img_list)  # type: ignore
