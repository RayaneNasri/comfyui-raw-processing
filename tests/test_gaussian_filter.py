...
from src.algorithms.denoising import gaussian_filter
...
def test_gaussian_filter_valid_zero_ksize():
    ...
    assert result.shape == img.shape, "Output shape must match the input shape when ksize is (0,0)"
...
@pytest.mark.parametrize("invalid_ksize", [
...
def test_gaussian_filter_gpu_tensor():
    ...
    try:
        ...
        assert result.device == img.device, "If input is on GPU, output should return to GPU"
    except TypeError:
        pytest.fail("Function crashed on GPU tensor. It lacks proper tensor-to-numpy conversion logic.")