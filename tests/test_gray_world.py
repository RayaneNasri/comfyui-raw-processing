...
def test_color_correction():
    ...
    assert torch.allclose(mean_r, mean_g), f"Red ({mean_r}) and Green ({mean_g}) means differ!"
    assert torch.allclose(mean_g, mean_b), f"Green ({mean_g}) and Blue ({mean_b}) means differ!"
