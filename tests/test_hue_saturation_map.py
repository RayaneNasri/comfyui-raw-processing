...
D50_WHITE_XYZ = torch.tensor([0.3457, 0.3585, 0.2958], dtype = torch.float32)
...
def _assert_tensors_close(actual: Tensor, expected: Tensor, atol: float = 1e-3, msg: str = ""):
    ...
    lines = [
        ...
        f"  first 5 failures (index | actual | expected | diff):",
    ]
    for idx in failing_indices[:5]:
        i = tuple(idx.tolist())
        lines.append(f"     [{i}]  {actual[i].item():.6f}  vs  {expected[i].item():.6f}  (diff {diff[i].item():.6f})")
    
    pytest.fail("\n".join(lines))

def _make_d50_grey(value: float = 0.5, height: int = 64, width: int = 64) -> Tensor:
    neutral = D50_WHITE_XYZ / D50_WHITE_XYZ.max() * value
    return neutral.view(1, 1, 3).expand(height, width, 3).clone()

def _make_d50_random_image(height: int = 64, width: int = 64, seed: int = 0) -> Tensor:
    torch.manual_seed(seed)
    rgb = torch.rand(height, width, 3, dtype = torch.float32)
    return rgb
...
class TestOutputShape: 
    def test_same_shape_as_input(self):
        ...
        out = apply_hue_sat_map(
            ...
            _identity_forward_matrix(), _identity_forward_matrix(),
...
class TestHueCorrection:
    ...
    def test_hue_shift_360_is_identity(self):
        ...
        out_360 = apply_hue_sat_map(
            ...
            _identity_color_matrix(), _identity_color_matrix(),
            _identity_forward_matrix(), _identity_forward_matrix(),
            lut_360, lut_360, ILLUM_D65, ILLUM_D65,
        )
    ...
    def test_grey_image_unaffected_by_hue_shift(self):
        ...
        grey_val = 0.5
        grey_xyz = (neutral_xyz / neutral_xyz.max() * grey_val)
        img = grey_xyz.view(1, 1, 3).expand(64, 64, 3).clone()
    ...
    def test_opposite_hue_shifts_cancel(self):
        ...
        mid = apply_hue_sat_map(
            ...
            _identity_color_matrix(), _identity_color_matrix(),
            _identity_forward_matrix(), _identity_forward_matrix(),
            lut_pos, lut_pos, ILLUM_D65, ILLUM_D65,
        )
        out = apply_hue_sat_map(
            mid, _unit_wb_gains(),
            _identity_color_matrix(), _identity_color_matrix(),
            _identity_forward_matrix(), _identity_forward_matrix(),
            lut_neg, lut_neg, ILLUM_D65, ILLUM_D65,
        )
...
class TestSaturationCorrection:
    ...
    def test_zero_saturation_produces_grey(self):
        ...
        if highly_sat.any():
            reduction = (sat_out[highly_sat] / sat_in[highly_sat].clamp(min = 1e-6)).mean()
    ...
    def test_saturation_sigma_less_than_1_reduces_saturation(self):
        ...
        out_full = apply_hue_sat_map(
            ...
            _identity_color_matrix(), _identity_color_matrix(),
            _identity_forward_matrix(), _identity_forward_matrix(),
            lut_full, lut_full, ILLUM_D65, ILLUM_D65,
        )
        out_half = apply_hue_sat_map(
            img, _unit_wb_gains(),
            _identity_color_matrix(), _identity_color_matrix(),
            _identity_forward_matrix(), _identity_forward_matrix(),
            lut_half, lut_half, ILLUM_D65, ILLUM_D65,
        )
    ...
    def test_opposite_saturation_scales_cancel(self):
        ...
        mid = apply_hue_sat_map(
            ...
            _identity_color_matrix(), _identity_color_matrix(),
            _identity_forward_matrix(), _identity_forward_matrix(),
            lut_up, lut_up, ILLUM_D65, ILLUM_D65,
        )
        out = apply_hue_sat_map(
            mid, _unit_wb_gains(),
            _identity_color_matrix(), _identity_color_matrix(),
            _identity_forward_matrix(), _identity_forward_matrix(),
            lut_down, lut_down, ILLUM_D65, ILLUM_D65,
        )
        _assert_tensors_close(out, img, atol = 2e-3)
...
class TestValueCorrection:
    ...
    def test_opposite_value_scales_cancel(self):
        ...
        white = neutral_xyz / neutral_xyz.max()
        torch.manual_seed(9)
        brightness = torch.rand(64, 64, 1, dtype = torch.float32) * 0.4  # keep dark so ×2 won't clip
        img = (white.view(1, 1, 3) * brightness).clone()
