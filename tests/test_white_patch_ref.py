...
def test_color_correction():
    ...
    out = white_patch_ref(tinted_img, 1.0)

    # 4. Assertions
    # The maximums of the output image MUST all be 1.0
    assert torch.allclose(out[..., 0].max(), torch.tensor(1.0)), (
        "Max of Red channel is not 1.0"
    )
    assert torch.allclose(out[..., 1].max(), torch.tensor(1.0)), (
        "Max of Green channel is not 1.0"
    )
    assert torch.allclose(out[..., 2].max(), torch.tensor(1.0)), (
        "Max of Blue channel is not 1.0"
    )
    ...
    assert_close(
        out[1, 1, :],
        expected_pixel,
        msg="Intermediate values were not scaled correctly.",
    )
...
def test_percentil_ignores_outliers():
    ...
    assert_close(
        out[5, 5, :],
        expected_base_pixel,
        msg="The percentile algorithm did not ignore the outlier.",
    )
